...
value_t CLJ_NIL, CLJ_T, CLJ_F, CLJ_EOF, QUOTE;
...
static clj_readstate_t *readstate = NULL;

static void free_readstate(clj_readstate_t *rs)
{
...
clj_exception_context_t *clj_ctx = NULL;
uint32_t clj_throwing_frame=0;  // active frame when exception was thrown
value_t clj_lasterror;

#define CLJ_TRY \
  clj_exception_context_t _ctx; int l__tr, l__ca; \
  _ctx.sp=SP; _ctx.frame=curr_frame; _ctx.rdst=readstate; _ctx.prev=clj_ctx; \
  _ctx.ngchnd = N_GCHND; clj_ctx = &_ctx;                                    \
  if (!setjmp(_ctx.buf)) \
    for (l__tr=1; l__tr; l__tr=0, (void)(clj_ctx=clj_ctx->prev))

#define CLJ_CATCH \
  else \
    for(l__ca=1; l__ca; l__ca=0, \
      clj_lasterror=CLJ_NIL,clj_throwing_frame=0,SP=_ctx.sp,curr_frame=_ctx.frame)

void clj_savestate(clj_exception_context_t *_ctx)
{
    _ctx->sp = SP;
...
void clj_restorestate(clj_exception_context_t *_ctx)
{
    clj_lasterror = CLJ_NIL;
    clj_throwing_frame = 0;
    SP = _ctx->sp;
    curr_frame = _ctx->frame;
}

void clj_raise(value_t e)
{
    clj_lasterror = e;
    // unwind read state
    while (readstate != clj_ctx->rdst) {
        free_readstate(readstate);
        readstate = readstate->prev;
    }
    if (clj_throwing_frame == 0)
        clj_throwing_frame = curr_frame;
    N_GCHND = clj_ctx->ngchnd;
    clj_exception_context_t *thisctx = clj_ctx;
    if (clj_ctx->prev)   // don't throw past toplevel
        clj_ctx = clj_ctx->prev;
...
void lerror(value_t e, const char *msg)
{
    ...
    e = POP();
    clj_raise(clj_list2(e, m));
}

void type_error(char *fname, char *expected, value_t got)
{
    clj_raise(clj_listn(4, TypeError, symbol(fname), symbol(expected), got));
}

void bounds_error(char *fname, value_t arr, value_t ind)
{
    clj_raise(clj_listn(4, BoundsError, symbol(fname), arr, ind));
}

// safe cast operators --------------------------------------------------------

#define isstring clj_isstring
...
int clj_is_keyword_name(char *str, size_t len)
{
...
value_t clj_gensym(value_t *args, uint32_t nargs)
{
...
int clj_isgensym(value_t v)
{
    return isgensym(v);
}

static value_t clj_gensymp(value_t *args, u_int32_t nargs)
{
    argcount("gensym?", nargs, 1);
    return isgensym(args[0]) ? CLJ_T : CLJ_F;
}
...
value_t alloc_vector(size_t n, int init)
{
    ...
    if (init) {
        unsigned int i;
        for(i=0; i < n; i++)
            vector_elt(v, i) = CLJ_UNSPECIFIED;
    }
    return v;
}
...
void clj_free_gc_handles(uint32_t n)
{
    assert(N_GCHND >= n);
    N_GCHND -= n;
}
...
void gc(int mustgrow)
{
    ...
    if (clj_throwing_frame > curr_frame) {
        top = clj_throwing_frame - 4;
        f = Stack[clj_throwing_frame-4];
    }
    else {
        top = SP;
        f = curr_frame;
    ...
    clj_lasterror = relocate(clj_lasterror);
    ...
    if (grew || ((lim-curheap) < (int)(heapsize/5)) || mustgrow) {
        ...
        if (temp == NULL)
            clj_raise(memory_exception_value);
        tospace = temp;
        if (grew) {
            heapsize*=2;
...
value_t clj_apply(value_t f, value_t l)
{
    value_t v = l;
    uint32_t n = SP;
...
value_t clj_applyn(uint32_t n, value_t f, ...)
{
    va_list ap;
    va_start(ap, f);
    size_t i;
...
value_t clj_list2(value_t a, value_t b)
{
    PUSH(a);
    PUSH(b);
...
value_t clj_cons(value_t a, value_t b)
{
    PUSH(a);
    PUSH(b);
    value_t c = mk_cons();
...
static value_t apply_cl(uint32_t nargs)
{
 ...
 apply_cl_top:
    ...
    dispatch:
        ...
        OP(OP_BRF)
            v = POP();
            if (v == CLJ_F) ip += (ptrint_t)GET_INT16(ip);
            else ip += 2;
            NEXT_OP;
        OP(OP_BRT)
            v = POP();
        ...
        OP(OP_BRFL)
            v = POP();
            if (v == CLJ_F) ip += (ptrint_t)GET_INT32(ip);
            else ip += 4;
            NEXT_OP;
        OP(OP_BRTL)
            v = POP();
        ...
        OP(OP_EQ)
            Stack[SP-2] = ((Stack[SP-2] == Stack[SP-1]) ? CLJ_T : CLJ_F);
            POPN(1); NEXT_OP;
        OP(OP_EQV)
            if (Stack[SP-2] == Stack[SP-1]) {
                ...
                v = (compare_(Stack[SP-2], Stack[SP-1], 1)==0 ? CLJ_T : CLJ_F);
            }
            Stack[SP-2] = v; POPN(1);
            NEXT_OP;
        OP(OP_EQUAL)
            ...
            NEXT_OP;
        OP(OP_PAIRP)
            Stack[SP-1] = (iscons(Stack[SP-1]) ? CLJ_T : CLJ_F); NEXT_OP;
        OP(OP_ATOMP)
            Stack[SP-1] = (iscons(Stack[SP-1]) ? CLJ_F : CLJ_T); NEXT_OP;
        OP(OP_NOT)
            Stack[SP-1] = ((Stack[SP-1]==CLJ_F) ? CLJ_T : CLJ_F); NEXT_OP;
        OP(OP_NULLP)
            Stack[SP-1] = ((Stack[SP-1]==NIL) ? CLJ_T : CLJ_F); NEXT_OP;
        OP(OP_BOOLEANP)
            v = Stack[SP-1];
            Stack[SP-1] = ((v == CLJ_T || v == CLJ_F) ? CLJ_T:CLJ_F); NEXT_OP;
        OP(OP_SYMBOLP)
            Stack[SP-1] = (issymbol(Stack[SP-1]) ? CLJ_T : CLJ_F); NEXT_OP;
        OP(OP_NUMBERP)
            v = Stack[SP-1];
            Stack[SP-1] = (clj_isnumber(v) ? CLJ_T:CLJ_F); NEXT_OP;
        OP(OP_FIXNUMP)
            Stack[SP-1] = (isfixnum(Stack[SP-1]) ? CLJ_T : CLJ_F); NEXT_OP;
        OP(OP_BOUNDP)
            sym = tosymbol(Stack[SP-1], "bound?");
            Stack[SP-1] = ((sym->binding == UNBOUND) ? CLJ_F : CLJ_T);
            NEXT_OP;
        OP(OP_BUILTINP)
            v = Stack[SP-1];
            Stack[SP-1] = (isbuiltin(v) || iscbuiltin(v)) ? CLJ_T : CLJ_F;
            NEXT_OP;
        OP(OP_FUNCTIONP)
            v = Stack[SP-1];
            Stack[SP-1] = ((tag(v)==TAG_FUNCTION &&
                            (uintval(v)<=OP_ASET || v>(N_BUILTINS<<3))) ||
                           iscbuiltin(v)) ? CLJ_T : CLJ_F;
            NEXT_OP;
        OP(OP_VECTORP)
            Stack[SP-1] = (isvector(Stack[SP-1]) ? CLJ_T : CLJ_F); NEXT_OP;
        ...
        apply_sub:
            ...
            if (n == 1) goto do_neg;
            i = SP-n;
            // we need to pass the full arglist on to clj_add_any
            // so it can handle rest args properly
            PUSH(Stack[i]);
            Stack[i] = fixnum(0);
            Stack[i+1] = clj_neg(clj_add_any(&Stack[i], n, 0));
            Stack[i] = POP();
            v = clj_add_any(&Stack[i], 2, 0);
            POPN(n);
            PUSH(v);
            NEXT_OP;
        OP(OP_NEG)
        ...
        apply_mul:
            ...
            for (; i < SP; i++) {
                if (isfixnum(Stack[i])) {
                    ...
                    v = clj_mul_any(&Stack[i], SP-i, accum);
                    break;
                }
            }
            if (i == SP) {
        ...
        apply_div:
            i = SP-n;
            if (n == 1) {
                Stack[SP-1] = clj_div2(fixnum(1), Stack[i]);
            }
            else {
                if (n > 2) {
                    PUSH(Stack[i]);
                    Stack[i] = fixnum(1);
                    Stack[i+1] = clj_mul_any(&Stack[i], n, 1);
                    Stack[i] = POP();
                }
                v = clj_div2(Stack[i], Stack[i+1]);
                POPN(n);
                PUSH(v);
            }
            NEXT_OP;
        ...
        OP(OP_NUMEQ)
            ...
            if (bothfixnums(v, e))
                ...
                v = (!numeric_compare(v,e,1,0,"=")) ? CLJ_T : CLJ_F;
            POPN(1);
            Stack[SP-1] = v;
            NEXT_OP;
        OP(OP_LT)
            if (bothfixnums(Stack[SP-2], Stack[SP-1])) {
                v = (numval(Stack[SP-2]) < numval(Stack[SP-1])) ? CLJ_T : CLJ_F;
            }
            else {
                v = (numval(clj_compare(Stack[SP-2], Stack[SP-1])) < 0) ?
                    CLJ_T : CLJ_F;
            }
            POPN(1);
            Stack[SP-1] = v;
        ...
        OP(OP_LOADT) PUSH(CLJ_T); NEXT_OP;
        OP(OP_LOADF) PUSH(CLJ_F); NEXT_OP;
        ...
        do_loadg:
            ...
            if (sym->binding == UNBOUND)
                clj_raise(clj_list2(UnboundError, v));
            PUSH(sym->binding);
            NEXT_OP;
...
static value_t clj_function(value_t *args, uint32_t nargs)
{
    if (nargs == 1 && issymbol(args[0]))
        return clj_builtin(args, nargs);
    if (nargs < 2 || nargs > 4)
...
static value_t clj_function_code(value_t *args, uint32_t nargs)
{
...
static value_t clj_function_vals(value_t *args, uint32_t nargs)
{
...
static value_t clj_function_env(value_t *args, uint32_t nargs)
{
...
static value_t clj_function_name(value_t *args, uint32_t nargs)
{
...
value_t clj_copylist(value_t *args, u_int32_t nargs)
{
    argcount("copy-list", nargs, 1);
    return copy_list(args[0]);
}

value_t clj_append(value_t *args, u_int32_t nargs)
{
    if (nargs == 0)
        return NIL;
    value_t first=NIL, lst, lastcons=NIL;
    clj_gc_handle(&first);
    clj_gc_handle(&lastcons);
    uint32_t i=0;
    while (1) {
        lst = args[i++];
        if (i >= nargs) break;
...
value_t clj_liststar(value_t *args, u_int32_t nargs)
{
...
value_t clj_stacktrace(value_t *args, u_int32_t nargs)
{
    (void)args;
    argcount("stacktrace", nargs, 0);
    return _stacktrace(clj_throwing_frame ? clj_throwing_frame : curr_frame);
}

value_t clj_map1(value_t *args, u_int32_t nargs)
{
    if (nargs < 2)
...
static builtinspec_t core_builtin_info[] = {
    { "function", clj_function },
    { "function:code", clj_function_code },
    { "function:vals", clj_function_vals },
    { "function:env", clj_function_env },
    { "function:name", clj_function_name },
    { "stacktrace", clj_stacktrace },
    { "gensym", clj_gensym },
    { "gensym?", clj_gensymp },
    { "hash", clj_hash },
    { "copy-list", clj_copylist },
    { "append", clj_append },
    { "list*", clj_liststar },
    { "map", clj_map1 },
    { NULL, NULL }
...
static void lisp_init(size_t initial_heapsize)
{
    ...
    CLJ_NIL = NIL = builtin(OP_THE_EMPTY_LIST);
    CLJ_T = builtin(OP_BOOL_CONST_T);
    CLJ_F = builtin(OP_BOOL_CONST_F);
    CLJ_EOF = builtin(OP_EOF_OBJECT);
    ...
    set(printprettysym=symbol("*print-pretty*"), CLJ_T);
    set(printreadablysym=symbol("*print-readably*"), CLJ_T);
    set(printwidthsym=symbol("*print-width*"), fixnum(SCR_WIDTH));
    set(printlengthsym=symbol("*print-length*"), CLJ_F);
    set(printlevelsym=symbol("*print-level*"), CLJ_F);
    ...
    memory_exception_value = clj_list2(MemoryError,
...
value_t clj_toplevel_eval(value_t expr)
{
    return clj_applyn(1, symbol_value(evalsym), expr);
}

void clj_init(size_t initial_heapsize)
{
#ifdef BOEHM_GC
    GC_init();
#endif
...
int clj_load_system_image(value_t sys_image_iostream)
{
    value_t e;
    int saveSP;
    symbol_t *sym;
