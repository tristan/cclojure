...
static value_t do_read_sexpr(value_t label)
{
    ...
    case TOK_SHARPSYM:
        ...
        if (sym == vu8sym) {
            sym = arraysym;
            Stack[SP-1] = clj_cons(uint8sym, Stack[SP-1]);
        }
        else if (sym == fnsym) {
            sym = FUNCTION;
        }
        v = symbol_value(sym);
        if (v == UNBOUND)
            clj_raise(clj_list2(UnboundError, sym));
        return clj_apply(v, POP());
    case TOK_OPENB:
    ...
    case TOK_GENSYM:
        ...
        if (*pv == (value_t)HT_NOTFOUND)
            *pv = clj_gensym(NULL, 0);
        return *pv;
    case TOK_DOUBLEQUOTE:
        return read_string();
    }
    return CLJ_UNSPECIFIED;
}

value_t clj_read_sexpr(value_t f)
{
    value_t v;
    clj_readstate_t state;
