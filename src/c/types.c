...
cljtype_t *get_type(value_t t)
{
    ...
    ft = (cljtype_t*)malloc(sizeof(cljtype_t));
    ft->type = t;
    if (issymbol(t)) {
    ...
    if (iscons(t)) {
        if (isarray) {
            cljtype_t *eltype = get_type(car_(cdr_(t)));
            if (eltype->size == 0) {
                free(ft);
...
{
    cljtype_t *et = get_type(eltype);
    if (et->artype != NULL)
        return et->artype;
    return get_type(clj_list2(arraysym, eltype));
}

cljtype_t *define_opaque_type(value_t sym, size_t sz, cvtable_t *vtab,
                             cvinitfunc_t init)
{
    cljtype_t *ft = (cljtype_t*)malloc(sizeof(cljtype_t));
    ft->type = sym;
    ft->size = sz;
    ft->numtype = N_NUMTYPES;
    ft->vtable = vtab;
    ...
    for(i=0; i < h->size; i+=2) {
        if (h->table[i] != HT_NOTFOUND) {
            ...
            if (h->table[i+1] != HT_NOTFOUND)
                ((cljtype_t*)h->table[i+1])->type = (value_t)nv;
        }
    }
}
