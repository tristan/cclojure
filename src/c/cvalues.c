...
static cljtype_t *int8type, *uint8type;
static cljtype_t *int16type, *uint16type;
static cljtype_t *int32type, *uint32type;
static cljtype_t *int64type, *uint64type;
static cljtype_t *longtype, *ulongtype;
static cljtype_t *floattype, *doubletype;
       cljtype_t *bytetype, *wchartype;
       cljtype_t *stringtype, *wcstringtype;
       cljtype_t *builtintype;

static void cvalue_init(cljtype_t *type, value_t v, void *dest);
...
{
    cv->type = (cljtype_t*)(((uptrint_t)cv->type) | CV_OWNED_BIT);
    add_finalizer(cv);
}
...
static value_t cprim(cljtype_t *type, size_t sz)
{
...
value_t cvalue(cljtype_t *type, size_t sz)
{
    cvalue_t *pcv;
    int str=0;
...
value_t cvalue_from_data(cljtype_t *type, void *data, size_t sz)
{
    value_t cv;
    cv = cvalue(type, sz);
...
// into; NIL if none.
value_t cvalue_from_ref(cljtype_t *type, void *ptr, size_t sz, value_t parent)
{
    cvalue_t *pcv;
    value_t cv;
    ...
    pcv->type = type;
    if (parent != NIL) {
        pcv->type = (cljtype_t*)(((uptrint_t)pcv->type) | CV_PARENT_BIT);
        pcv->parent = parent;
    }
...
static int cvalue_##ctype##_init(cljtype_t *type, value_t arg,   \
                                 void *dest)                    \
{                                                               \
    clj_##ctype##_t n=0;                                         \
    (void)type;                                                 \
    if (isfixnum(arg)) {                                        \
        ...
        void *p = cp_data(cp);                                  \
        n = (clj_##ctype##_t)conv_to_##cnvt(p, cp_numtype(cp));  \
    }                                                           \
    else {                                                      \
        return 1;                                               \
    }                                                           \
    *((clj_##ctype##_t*)dest) = n;                               \
    return 0;                                                   \
}
...
{                                                                       \
    ...
    value_t cp = cprim(typenam##type, sizeof(clj_##ctype##_t));          \
...
value_t mk_##typenam(clj_##ctype##_t n)                                  \
{                                                                       \
    value_t cp = cprim(typenam##type, sizeof(clj_##ctype##_t));          \
    *(clj_##ctype##_t*)cp_data((cprim_t*)ptr(cp)) = n;                   \
    return cp;                                                          \
}
...
static int cvalue_enum_init(cljtype_t *ft, value_t arg, void *dest)
{
    int n=0;
    value_t syms;
...
value_t cvalue_enum(value_t *args, u_int32_t nargs)
{
    ...
    value_t type = clj_list2(enumsym, args[0]);
    cljtype_t *ft = get_type(type);
...
static int cvalue_array_init(cljtype_t *ft, value_t arg, void *dest)
{
    value_t type = ft->type;
    size_t elsize, i, cnt, sz;
    cljtype_t *eltype = ft->eltype;
    ...
    if (isvector(arg)) {
        ...
        if (isarray(arg)) {
            cljtype_t *aet = cv_class(cv)->eltype;
            if (aet == eltype) {
                if (cv_len(cv) == sz)
...
value_t cvalue_array(value_t *args, u_int32_t nargs)
{
    ...
    cnt = nargs - 1;
    cljtype_t *type = get_array_type(args[0]);
    elsize = type->elsz;
    sz = elsize * cnt;
...
static size_t cvalue_struct_offs(value_t type, value_t field, int computeTotal,
                                 int *palign)
{
    value_t cljd = car(cdr_(type));
    size_t fsz, ssz = 0;
    int al;
    *palign = 0;

    while (iscons(cljd)) {
        fsz = ctype_sizeof(car(cdr(car_(cljd))), &al);

        ssz = LLT_ALIGN(ssz, al);
        if (al > *palign)
            *palign = al;

        if (!computeTotal && field==car_(car_(cljd))) {
            // found target field
            return ssz;
        }
...
value_t cvalue_copy(value_t v)
{
    ...
    if (!isinlined(cv)) {
        ...
        autorelease(ncv);
        if (hasparent(cv)) {
            ncv->type = (cljtype_t*)(((uptrint_t)ncv->type) & ~CV_PARENT_BIT);
            ncv->parent = NIL;
        }
    }
    else {
...
value_t clj_copy(value_t *args, u_int32_t nargs)
{
...
value_t clj_podp(value_t *args, u_int32_t nargs)
{
...
{
    char *data; ulong_t index;
    cljtype_t *eltype = cv_class((cvalue_t*)ptr(args[0]))->eltype;
    value_t el = 0;
...
value_t clj_builtin(value_t *args, u_int32_t nargs)
{
...
static value_t clj_logand(value_t *args, u_int32_t nargs);
static value_t clj_logior(value_t *args, u_int32_t nargs);
static value_t clj_logxor(value_t *args, u_int32_t nargs);
static value_t clj_lognot(value_t *args, u_int32_t nargs);
static value_t clj_ash(value_t *args, u_int32_t nargs);
...
static void cvalues_init()
{
    ...
    setc(stringtypesym, clj_list2(arraysym, bytesym));

    wcstringtypesym = symbol("*wcstring-type*");
    setc(wcstringtypesym, clj_list2(arraysym, wcharsym));
...
#define RETURN_NUM_AS(var, type) return(mk_##type((clj_##type##_t)var))
...
static value_t clj_add_any(value_t *args, u_int32_t nargs, fixnum_t carryIn)
{
    uint64_t Uaccum=0;
...
static value_t clj_neg(value_t n)
{
    if (isfixnum(n)) {
...
static value_t clj_mul_any(value_t *args, u_int32_t nargs, int64_t Saccum)
{
    uint64_t Uaccum=1;
    double Faccum=1;
    uint32_t i;
...
static value_t clj_div2(value_t a, value_t b)
{
    double da, db;
    int_t ai, bi;
    numerictype_t ta, tb;
...
static value_t clj_idiv2(value_t a, value_t b)
{
    int_t ai, bi;
    numerictype_t ta, tb;
    void *aptr, *bptr;
...
static value_t clj_bitwise_op(value_t a, value_t b, int opcode, char *fname)
{
    int_t ai, bi;
