...
void print_htable(value_t v, ios_t *f)
{
    ...
    size_t i;
    int first=1;
    clj_print_str("#table(", f);
    for(i=0; i < h->size; i+=2) {
        if (h->table[i+1] != HT_NOTFOUND) {
            if (!first) clj_print_str("  ", f);
            clj_print_child(f, (value_t)h->table[i]);
            clj_print_chr(' ', f);
            clj_print_child(f, (value_t)h->table[i+1]);
            first = 0;
        }
    }
    clj_print_chr(')', f);
...
value_t clj_tablep(value_t *args, uint32_t nargs)
{
    argcount("table?", nargs, 1);
    return ishashtable(args[0]) ? CLJ_T : CLJ_F;
}
...
value_t clj_table(value_t *args, uint32_t nargs)
{
...
// (put! table key value)
value_t clj_table_put(value_t *args, uint32_t nargs)
{
...
{
    lerrorf(clj_list2(KeyError, key), "%s: key not found", fname);
}

// (get table key [default])
value_t clj_table_get(value_t *args, uint32_t nargs)
{
    if (nargs != 3)
...
// (has? table key)
value_t clj_table_has(value_t *args, uint32_t nargs)
{
    argcount("has", nargs, 2);
    htable_t *h = totable(args[0], "has");
    return equalhash_has(h, (void*)args[1]) ? CLJ_T : CLJ_F;
}

// (del! table key)
value_t clj_table_del(value_t *args, uint32_t nargs)
{
...
value_t clj_table_foldl(value_t *args, uint32_t nargs)
{
