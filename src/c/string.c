...
value_t clj_stringp(value_t *args, u_int32_t nargs)
{
    argcount("string?", nargs, 1);
    return clj_isstring(args[0]) ? CLJ_T : CLJ_F;
}

value_t clj_string_count(value_t *args, u_int32_t nargs)
{
    size_t start = 0;
    if (nargs < 1 || nargs > 3)
...
value_t clj_string_width(value_t *args, u_int32_t nargs)
{
...
value_t clj_string_reverse(value_t *args, u_int32_t nargs)
{
...
value_t clj_string_encode(value_t *args, u_int32_t nargs)
{
...
value_t clj_string_decode(value_t *args, u_int32_t nargs)
{
    int term=0;
    if (nargs == 2) {
        term = (args[1] != CLJ_F);
...
extern value_t clj_buffer(value_t *args, u_int32_t nargs);
extern value_t stream_to_string(value_t *ps);

value_t clj_string(value_t *args, u_int32_t nargs)
{
    if (nargs == 1 && clj_isstring(args[0]))
        return args[0];
    value_t arg, buf = clj_buffer(NULL, 0);
    ios_t *s = value2c(ios_t*,buf);
    uint32_t i;
    value_t oldpr = symbol_value(printreadablysym);
    value_t oldpp = symbol_value(printprettysym);
    set(printreadablysym, CLJ_F);
    set(printprettysym, CLJ_F);
...
value_t clj_string_split(value_t *args, u_int32_t nargs)
{
    ...
    value_t first=CLJ_NIL, c=CLJ_NIL, last;
    size_t junk;
    clj_gc_handle(&first);
    clj_gc_handle(&last);
    ...
    do {
        ...
        c = clj_cons(cvalue_string(ssz), CLJ_NIL);
...
value_t clj_string_sub(value_t *args, u_int32_t nargs)
{
    if (nargs != 2)
...
value_t clj_string_char(value_t *args, u_int32_t nargs)
{
...
value_t clj_char_upcase(value_t *args, u_int32_t nargs)
{
...
value_t clj_char_downcase(value_t *args, u_int32_t nargs)
{
...
value_t clj_string_find(value_t *args, u_int32_t nargs)
{
    char cbuf[8];
    size_t start = 0;
    if (nargs == 3)
...
value_t clj_string_inc(value_t *args, u_int32_t nargs)
{
    if (nargs < 2 || nargs > 3)
...
value_t clj_string_dec(value_t *args, u_int32_t nargs)
{
    if (nargs < 2 || nargs > 3)
...
value_t clj_numbertostring(value_t *args, u_int32_t nargs)
{
    if (nargs < 1 || nargs > 2)
    ...
    if (numval(clj_compare(args[0],fixnum(0))) < 0) {
        num = -num;
        neg = 1;
    }
...
value_t clj_stringtonumber(value_t *args, uint32_t nargs)
{
    if (nargs < 1 || nargs > 2)
...
static builtinspec_t stringfunc_info[] = {
    { "string", clj_string },
    { "string?", clj_stringp },
    { "string.count", clj_string_count },
    { "string.width", clj_string_width },
    { "string.split", clj_string_split },
    { "string.sub", clj_string_sub },
    { "string.find", clj_string_find },
    { "string.char", clj_string_char },
    { "string.inc", clj_string_inc },
    { "string.dec", clj_string_dec },
    { "string.reverse", clj_string_reverse },
    { "string.encode", clj_string_encode },
    { "string.decode", clj_string_decode },

    { "char.upcase", clj_char_upcase },
    { "char.downcase", clj_char_downcase },

    { "number->string", clj_numbertostring },
    { "string->number", clj_stringtonumber },
