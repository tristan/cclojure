...
extern value_t CLJ_NIL, CLJ_T, CLJ_F, CLJ_EOF;

#define CLJ_UNSPECIFIED CLJ_T

/* read, eval, print main entry points */
value_t clj_read_sexpr(value_t f);
void clj_print(ios_t *f, value_t v);
...
value_t clj_compare(value_t a, value_t b);  // -1, 0, or 1
value_t clj_equal(value_t a, value_t b);    // T or nil
...
/* error handling */
typedef struct _clj_readstate_t {
    htable_t backrefs;
    htable_t gensyms;
    value_t source;
    struct _clj_readstate_t *prev;
} clj_readstate_t;
...
typedef struct _ectx_t {
    ...
    uint32_t frame;
    uint32_t ngchnd;
    clj_readstate_t *rdst;
    struct _ectx_t *prev;
} clj_exception_context_t;

extern clj_exception_context_t *clj_ctx;
extern uint32_t clj_throwing_frame;
extern value_t clj_lasterror;

#define CLJ_TRY_EXTERN                                                   \
  clj_exception_context_t _ctx; int l__tr, l__ca;                        \
  clj_savestate(&_ctx); clj_ctx = &_ctx;                                  \
  if (!setjmp(_ctx.buf))                                                \
    for (l__tr=1; l__tr; l__tr=0, (void)(clj_ctx=clj_ctx->prev))

#define CLJ_CATCH_EXTERN \
  else \
    for(l__ca=1; l__ca; l__ca=0, clj_restorestate(&_ctx))
...
void clj_raise(value_t e) __attribute__ ((__noreturn__));
...
void clj_print_chr(char c, ios_t *f);
void clj_print_str(char *s, ios_t *f);
void clj_print_child(ios_t *f, value_t v);

typedef int (*cvinitfunc_t)(struct _cljtype_t*, value_t, void*);

typedef struct _cljtype_t {
    value_t type;
    numerictype_t numtype;
    size_t size;
    size_t elsz;
    cvtable_t *vtable;
    struct _cljtype_t *eltype;  // for arrays
    struct _cljtype_t *artype;  // (array this)
    int marked;
    cvinitfunc_t init;
} cljtype_t;
...
#define cv_class(cv)   ((cljtype_t*)(((uptrint_t)(cv)->type)&~3))
...
typedef int8_t   clj_int8_t;
typedef uint8_t  clj_uint8_t;
typedef int16_t  clj_int16_t;
typedef uint16_t clj_uint16_t;
typedef int32_t  clj_int32_t;
typedef uint32_t clj_uint32_t;
typedef int64_t  clj_int64_t;
typedef uint64_t clj_uint64_t;
typedef char     clj_char_t;
typedef char     char_t;
typedef long     clj_long_t;
typedef long     long_t;
typedef unsigned long clj_ulong_t;
typedef unsigned long ulong_t;
typedef double   clj_double_t;
typedef float    clj_float_t;
...
extern cljtype_t *bytetype, *wchartype;
extern cljtype_t *stringtype, *wcstringtype;
extern cljtype_t *builtintype;
...
cljtype_t *get_type(value_t t);
cljtype_t *get_array_type(value_t eltype);
cljtype_t *define_opaque_type(value_t sym, size_t sz, cvtable_t *vtab,
                             cvinitfunc_t init);

value_t mk_double(clj_double_t n);
value_t mk_float(clj_float_t n);
...
/* builtins */
value_t clj_hash(value_t *args, u_int32_t nargs);
...
int clj_load_system_image(value_t ios);
